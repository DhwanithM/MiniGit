...
def store_object(path, repository_path="."):
    ...
    if not minigit_path.is_dir():
        return False, "Error: MiniGit repository not found. Run 'python minigit.py init' first.", None

    if not source_path.is_file():
        return False, f"Error: file not found: {path}", None
    ...
    return True, f"Stored object {digest} from {path}", digest
